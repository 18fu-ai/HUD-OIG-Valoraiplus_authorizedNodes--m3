...
// Memoized Neural Network Background
...
export default function NewtChatRuntime() {
  ...
  const [exportTimestamp, setExportTimestamp] = useState<string>(new Date().toISOString());
  ...
  // Run firewall check on mount
  useEffect(() => {
    const signal: FirewallSignal = {
      id: 'runtime-signal',
      lineage: 'SAINT_PAUL_55116_SOVEREIGN',
      type: 'RUNTIME_CONFIRMED',
    };

    const result = waterfallFirewall(signal);
  ...
  // Block if firewall denies
  if (firewallResult && firewallResult.decision === 'BLOCK') {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <Card className="border-2 border-red-800 bg-red-950/50 p-8 max-w-md">
          <div className="text-center">
            <AlertTriangle className="w-12 h-12 text-red-500 mx-auto mb-4" />
            <h1 className="text-xl font-bold text-red-400 mb-2">ACCESS DENIED</h1>
            <p className="text-red-500 font-mono text-sm">{firewallResult.reason}</p>
          </div>
        </Card>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-emerald-400 font-mono relative">
      ...
      <main className="container mx-auto px-4 py-6 relative z-10 max-w-4xl">
        ...
        {/* Firewall Status */}
        {firewallResult && (
          <div className="mb-4 p-2 bg-emerald-950/30 border border-emerald-800 rounded flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Shield className="w-4 h-4 text-emerald-500" />
              <span className="text-xs text-emerald-500 font-mono">FIREWALL: {firewallResult.decision}</span>
            </div>
            <span className="text-[10px] text-emerald-700">{firewallResult.reason}</span>
          </div>
        )}
