...
export async function POST(request: NextRequest) {
  ...
  const insertPayload = {
    ...
    request_class:         payload.request_category ?? 'unknown', // backward compat
    ...
    source:                payload.source                            ?? 'vercel_edge_middleware_phase2',
    raw_ip_stored:         false,
...
// ---------------------------------------------------------------------------
// Validation — all three hashes are now required per spec
// ---------------------------------------------------------------------------
function validatePayload(payload: AccessPayload): string | null {
  ...
  const validCategories: RequestCategory[] = [
    'page', 'pdf', 'api', 'asset', 'document', 'search', 'unknown',
  ]
  if (payload.request_category && !validCategories.includes(payload.request_category))
    return 'invalid_request_category'
...
// ---------------------------------------------------------------------------
// Supabase error normalizer — detects table-missing AND column-missing
// ---------------------------------------------------------------------------
function normalizeSupabaseError(error: {
  ...
  if (tableMissing)  return { reason: 'table_missing_run_migration_004', code: error.code ?? 'unknown' }
  if (columnMissing) return { reason: 'column_missing_run_phase2_patch',  code: error.code ?? 'unknown' }
